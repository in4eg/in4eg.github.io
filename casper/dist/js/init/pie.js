  ...
  if ($('.ico-section').hasClass('active')) {
    ...
    if (i === 1) {
      new Chart(document.getElementById('myChart'), {
        ...
        data: {
          labels: ['Pre-ico цена токена - $0,8', 'ICO цена токена - $1,6', 'РЕЗЕРВ вознаграждения крупных инвесторов', 'Команда', 'РЕЗЕРВ СИСТЕМЫ'],
          datasets: [
            {
...
watchPie = function() {
  var elementHeight, elementOffset, top;
  if ($('#main-page').length > 0) {
    top = $(document).scrollTop();
    elementOffset = $('.ico-section').offset().top;
    elementHeight = $(window).height();
    if (top + elementHeight - 150 > elementOffset && top < elementOffset + elementHeight - 150) {
      i = 2;
      new Chart(document.getElementById('myChart'), {
        type: 'pie',
        options: {
