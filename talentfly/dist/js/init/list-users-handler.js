var clearTime, clearUsersList, closeList, dataRequest, isEmpty, months, privateLinks, renderViewItem, usersList;
...
dataRequest = function(page) {
  var candidate, city, country, currentPageEl, email, interviewer, linkBox, role, url, withInterview, withoutInterview;
  clearUsersList();
...
renderViewItem = function(data) {
  var i, interviewItem, interviewsList, interviewsListBox, newItem, time;
  ...
  if (data.role === 'candidate' && !isEmpty(data.invite) && data.invite) {
  ...
  if (data.role === 'interviewer' && data.interviews.length > 0) {
...
$('.interviews-list.users').on('click', '.accordeon-title', function() {
  var content, contents, state, title;
