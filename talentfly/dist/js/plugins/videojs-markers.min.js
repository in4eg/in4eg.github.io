...
(function ($, videojs, undefined) {
  ...
  function registerVideoJsMarkersPlugin(options) {
    ...
    function getPosition(marker) {
      ...
      return marker.word_time / player.duration() * 100;
    }

    function createMarkerDiv(marker) {
      var markerDiv = $("<div class='vjs-marker'></div>");
      var progressLineWidth = $('.vjs-load-progress').width();
      // 300 = 3px *100% (3px = width of orange control circle)
    //   var difference = +(300/progressLineWidth).toFixed(2);
      markerDiv.css(setting.markerStyle).css({
        "margin-left": -parseFloat(markerDiv.css("width")) / 2 + 3 + 'px',
    ...
    function registerMarkerTipHandler(markerDiv) {
      markerDiv.on('mouseover', function () {
        ...
        if (!!markerTip) {
          ...
          markerTip.css({
            "left": getPosition(marker) + '%',
            "margin-left": -parseFloat(markerTip.width()) / 2 -5 + 3 + 'px',
            "visibility": "visible"
          });
        }
        
        $('.vjs-time-tooltip').css('display', 'none');
       
      });

      markerDiv.on('mouseout', function () {
        !!markerTip && markerTip.css("visibility", "hidden");
        $('.vjs-time-tooltip').css('display', 'block');
      });
    }
