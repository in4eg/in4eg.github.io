...
do titleFont = ->
	...
	if $(window).innerWidth() < 1260 and $(window).innerWidth() >= 1051
		if totalVal < 20
...
# count input value
$ document
	.on 'input change', '[data-characters]', ->
		maxLength = $(@).data('characters')
		textVal = $(@).val().trim().length

		if $(@).val().length >= 0
			$(@).siblings('.count-help').find('.count').html maxLength - textVal
		return
